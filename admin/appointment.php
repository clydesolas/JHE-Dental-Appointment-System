...
<div class='dashboard'>
    <div class="dashboard-nav">
        ...
        <nav class="dashboard-nav-list">
          ...
          if ($totaluser1>0){
             echo '<span class="mx-2 badge bg-danger top-0  rounded-pill">'. $totaluser1 .'</span>'; 
        ...
        <div class='dashboard-content'>
            <div class='container'>
                <div class='card'>
                    ...
                    <button type="button" href="appointment.php"class="btn btn-secondary rounded-0 text-nowrap"disabled >Pending  <?php
                        $select1="SELECT `status` FROM appointment WHERE `status`='Pending'";
                        $user1 = $con->query($select1) or die ($con->error);
                        $row = $user1->fetch_assoc();
                        $totaluser1 = $user1->num_rows;
                    ...
                    <a type="button" href="appointments-declined.php" class="btn btn-secondary text-decoration-none text-nowrap">Declined</a>
                    ...
                    <thead class="text-white" style="background-color:#305f82;">
                                ...
                                <tbody >
                                  ...
                                  <?php 
                                  $sqll = mysqli_query($con,"SELECT * FROM appointment where `status`= 'Pending'")or die(mysqli_error($con));
                                  while($row = mysqli_fetch_array($sqll)){
                                    ...
                                    <td>
                                         <a rel="tooltip"  title="View Appointment" id="<?php echo $appointment_id; ?>" href="#view<?php echo $appointment_id; ?>" data-bs-toggle="modal">View</a> 
                                         <?php include('modal_appointment-pending.php'); ?>
                                    </td>
