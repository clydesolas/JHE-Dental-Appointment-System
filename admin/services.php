    ...
    <div class="dashboard-nav">
        ...
        <nav class="dashboard-nav-list">
          ...
          if ($totaluser1>0){
            ...
            <a href="services.php" class="dashboard-nav-item active">
        ...
        <div class='dashboard-content'>
            <div class='container'>
                <div class='card'>
                    ...
                    <a data-bs-toggle="modal" style="text-decoration: none" href="#add_service" type="button" class="btn btn-primary mb-3"><i class="bi bi-plus-lg"></i> Add Service</a>
                    <br>
                    ...
                    <thead class="text-white" style="width:100%;background-color:#305f82;">
                                    <tr>
                                         <th>No.</th>
                                        <th>Service Offer</th>
                                        <th>Image</th>
                                ...
                                <tbody >
                                  ...
                                  <?php 
                                   $i = 1;
                                  $sqll = mysqli_query($con,"SELECT * FROM services ORDER BY `service` ASC")or die(mysqli_error($con));
                                  while($row = mysqli_fetch_array($sqll)){
									$service_id=$row['service_id']; 
                                    $number=0;?>
                                   
									 <tr class="del<?php echo $service_id ?>">
									      <td><?php echo $i++?></td>
                                    <td><?php echo $row['service']; ?></td> 
                                    <td><a rel="tooltip"  title="View Image" id="<?php echo $service_id; ?>"href="#imgview<?php echo $service_id; ?>" data-bs-toggle="modal" class="p-0 m-0 text-decoration-none"><?php if (!empty($row['service_imgpath'])){echo "<img src='image/".$row['service_imgpath']."'style='width:40px; height:40px' >"."&emsp;".$row['service_imgpath'];} ?></a></td> 
                                    <td width="110">
                                        <a rel="tooltip"  title="Delete" id="<?php echo $service_id; ?>"href="#delete<?php echo $service_id; ?>" data-bs-toggle="modal" class="btn btn-danger"><i class="bi bi-trash"></i></a>
                                        <a rel="tooltip"  title="Edit" id="<?php echo $service_id; ?>" href="#edit<?php echo $service_id; ?>" data-bs-toggle="modal" class="btn btn-success"><i class="bi bi-pencil-square"></i></a>                          
                                    </td>
...
$('#file').on('change', function() {
      ...
      $('#file1').on('change', function() {
          ...
          if (size > 5) {
            $("#output1").html('<b style="color:red">' +
                 'Image size is higher than 5MB'+'</b><br>Image size: ' + size + " MB");
              upl.value = "";
          } else {
              $("#output1").html('<b>' +
